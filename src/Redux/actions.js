...
export const checkEmail = async (email) => {
  try {
    const response = await axios.get(`http://localhost:3001/rickandmorty/register?email=${email}`);
...
export const createUser = (user) => {
  return async (dispatch) => {
    try {
      const response = await axios.post(`http://localhost:3001/rickandmorty/login`, user);
